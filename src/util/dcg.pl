:- module(dcg, [eol/2, intseq/3, intseq/4, call_dcg/4, sep_by/5, lines/3]).

:- use_module(library(dcg/basics), [eos/2, integer/3, string_without/4, string/3]).
...
lines([H|T]) --> string_without("\n", H), "\n", !, lines(T).
lines([H]) --> !, string(H), eos.
lines([]) --> !, eos.
